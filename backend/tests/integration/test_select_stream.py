...
class TestSelectStreamEndpoint:
    ...
    def test_fine_threshold_bfs_with_display_mapping(self, client):
        """Test BFS at fine threshold (100) — ADR-024 path at threshold 100."""
        ...
        with patch(
            ...
            # ADR-024: BFS at fine threshold (100), no display mapping needed
            assert "upstream_segment_indices" in data
            assert sorted(data["upstream_segment_indices"]) == [1, 2, 3]
    ...
    def test_coarse_threshold_uses_display_threshold(self, client):
        """Test coarse threshold (!=100) snaps to stream at display threshold."""
        cg = _make_mock_catchment_graph()
        ...
        # snap-to-stream returns segment at threshold 10000
        snap_result = MagicMock()
        snap_result.segment_idx = 12  # Maps to (10000, 12) → idx 2
        ...
        def execute_side_effect_coarse(query, params=None):
            result = MagicMock()
            query_str = str(query)

            if "ST_ClosestPoint" in query_str:
                result.fetchone.return_value = snap_result
            elif "stream_network" in query_str and "ST_DWithin" in query_str:
                result.fetchone.return_value = segment_result
            elif "ST_UnaryUnion" in query_str:
                result.fetchone.return_value = boundary_result
        ...
        mock_session.execute.side_effect = execute_side_effect_coarse
        ...
        with patch(
            ...
            assert response.status_code == 200
            data = response.json()
            # Coarse path: snap at display threshold 10000, BFS at 10000
            # Node (10000, 12) → idx 2, upstream {0,1,2} → [10, 11, 12]
            assert sorted(data["upstream_segment_indices"]) == [10, 11, 12]
            assert data["watershed"]["area_km2"] > 0

            app.dependency_overrides.clear()

    def test_fine_threshold_100_uses_adr024_logic(self, client):
        """Test threshold 100 uses ADR-024 fine-BFS logic."""
        cg = CatchmentGraph()
        n = 3
        cg._n = n
        cg._loaded = True
        ...
        cg._threshold_m2 = np.array([100, 100, 100], dtype=np.int32)
        cg._area_km2 = np.array([0.5, 0.8, 1.3], dtype=np.float32)
        cg._elev_min = np.array([140.0, 150.0, 130.0], dtype=np.float32)
        cg._elev_max = np.array([180.0, 190.0, 160.0], dtype=np.float32)
        cg._elev_mean = np.array([160.0, 170.0, 145.0], dtype=np.float32)
        cg._slope_mean = np.array([4.0, 5.0, 3.0], dtype=np.float32)
        cg._perimeter_km = np.array([3.0, 4.0, 5.0], dtype=np.float32)
        cg._stream_length_km = np.array([0.5, 0.7, 1.0], dtype=np.float32)
        cg._strahler = np.array([1, 1, 2], dtype=np.int8)
        cg._histograms = [
            {"base_m": 140, "interval_m": 1, "counts": [10, 20, 30]},
            {"base_m": 150, "interval_m": 1, "counts": [15, 25]},
        ...
        segment_result.id = 3
        segment_result.strahler_order = 2
        segment_result.length_m = 1000.0
        segment_result.upstream_area_km2 = 1.3
        segment_result.downstream_x = 639139.0
        segment_result.downstream_y = 486706.0

        # snap at threshold 100 returns segment 3
        snap_result = MagicMock()
        snap_result.segment_idx = 3  # Maps to (100, 3) → idx 2
        ...
        def execute_side_effect_fine(query, params=None):
            result = MagicMock()
            query_str = str(query)
        ...
        mock_session.execute.side_effect = execute_side_effect_fine
        ...
        with patch(
            ...
            assert response.status_code == 200
            data = response.json()
            # ADR-024: fine BFS at threshold 100
            # Node (100, 3) → idx 2, upstream {0,1,2} → [1, 2, 3]
            assert sorted(data["upstream_segment_indices"]) == [1, 2, 3]
            assert data["watershed"]["area_km2"] > 0
