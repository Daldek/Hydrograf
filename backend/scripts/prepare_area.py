...
Requires:
- Kartograf 0.4.1+ for data download
...
def main():
    ...
    hydro_group = parser.add_argument_group(
        "Hydrographic data (requires Kartograf 0.4.1+)"
    )
    ...
    landcover_group = parser.add_argument_group(
        "Land cover options (requires Kartograf 0.4.1+)"
    )
    ...
    if args.dry_run:
        ...
        if args.with_hydro:
            logger.info("Would also download BDOT10k hydrographic data for stream burning")
        if args.with_landcover:
